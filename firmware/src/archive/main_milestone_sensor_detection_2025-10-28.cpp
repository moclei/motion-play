/**
 * Motion Play v2.0 - MILESTONE BACKUP - October 28, 2025
 * 
 * MILESTONE: First successful VCNL4040 sensor detection and communication!
 * - Successfully detected TCA9548A multiplexer at 0x70
 * - Successfully detected PCA9546A multiplexer at 0x70 (on sensor board)
 * - Successfully detected and initialized VCNL4040 sensor on PCA channel 0
 * - Confirmed sensor readings: Proximity and Ambient light values
 * - Established full I2C communication chain: ESP32 -> TCA9548A -> PCA9546A -> VCNL4040
 *
 * This version represents the working foundation for sensor communication.
 * IC2 (second sensor) not detected - likely hardware assembly issue.
 *
 * Phase 4 Objectives ACHIEVED:
 * - ✅ I2C bus initialization and scanning
 * - ✅ TCA9548A multiplexer communication
 * - ✅ PCA9546A multiplexer communication through TCA
 * - ✅ VCNL4040 sensor detection and initialization
 * - ✅ Live sensor readings (proximity: 1, ambient: 70)
 * - ✅ Continuous sensor monitoring every 100ms
 *
 * Hardware: T-Display-S3 (ESP32-S3) with sensor PCB on TCA channel 0
 * Libraries:
 * - TFT_eSPI for display control
 * - robtillaart/TCA9548 for I2C multiplexer
 * - Adafruit VCNL4040 for proximity sensors
 */
...
bool initializeDisplay()
{
    ...
    // Enable backlight (critical for T-Display-S3)
...
void runPhase4Tests()
{
    ...
    // PCA Channel I2C Scanning (diagnostic)
    ...
    {
        logMessage("Skipping PCA scan - prerequisites failed", COLOR_YELLOW);
    }
...
void initializeI2C()
{
    ...
    Wire.setClock(100000); // 100kHz for better reliability with multiple pullups
    ...
    logMessage("Clock: 100kHz (for reliability)", COLOR_CYAN);
...
/**
 * Scan I2C devices on each PCA channel individually
 */
void scanPCAChannels()
{
    logMessage("=== PCA CHANNEL I2C SCAN ===", COLOR_CYAN);
    ...
    logMessage("TCA channel 0 selected for PCA scanning", COLOR_GREEN);
    delay(50); // Allow channel to stabilize

    // Scan each PCA channel
    for (int pca_channel = 0; pca_channel < 4; pca_channel++)
    {
        logMessage("=== Scanning PCA Channel " + String(pca_channel) + " ===", COLOR_YELLOW);
        ...
        delay(100); // Longer delay for device stabilization

        // Scan I2C addresses on this channel
        int deviceCount = 0;
        for (uint8_t addr = 0x08; addr <= 0x77; addr++)
        {
            ...
            {
                deviceCount++;
                String addr_str = "0x" + String(addr, HEX);
                if (addr < 16)
                    addr_str = "0x0" + String(addr, HEX);

                String device_name = "Unknown";
                if (addr == 0x60)
                    device_name = "VCNL4040?";
                else if (addr == 0x29)
                    device_name = "VCNL4040 Alt?";
                else if (addr == 0x13)
                    device_name = "VCNL4040 Alt2?";

                logMessage("  Found: " + addr_str + " (" + device_name + ")", COLOR_GREEN);
            }

            // Small delay every 16 addresses to prevent watchdog issues
            if (addr % 16 == 0)
            {
                delay(10);
                yield();
        ...
        if (deviceCount == 0)
        {
            logMessage("  No devices found on PCA ch" + String(pca_channel), COLOR_YELLOW);
        }
        else
        {
            logMessage("  Total devices on PCA ch" + String(pca_channel) + ": " + String(deviceCount), COLOR_CYAN);
        }

        // Disable this PCA channel before moving to next
        pca.disableAllChannels();
        delay(50);
    }
    ...
    pca.disableAllChannels();
    tca.disableAllChannels();
    logMessage("All channels disabled after scan", COLOR_YELLOW);

    logMessage("=== PCA CHANNEL SCAN COMPLETE ===", COLOR_CYAN);
}
